...
| **Insert en lote con filas de distinta forma** | `null value in column "had_seat" violates not-null constraint`, con la columna declarada `not null default false` | PostgREST recibe el array entero y monta UN insert con la UNIÓN de las claves. A la fila que no trae una clave le pone **NULL, no el DEFAULT**. Basta con que una fila del lote mencione la columna para tumbar a las demás. | En cualquier `.insert([...])` de varias filas, que **todas** lleven **todas** las claves, aunque vayan a `null`. Un objeto base + spread del caso particular lo resuelve y además se lee mejor. |
| **`grant` de menos no retira permisos** | Escribes `grant select, update, delete` a propósito, para que no haya INSERT, y el INSERT sigue concedido | Supabase deja `alter default privileges ... grant all on tables to anon, authenticated, service_role`. Toda tabla nueva del esquema `public` nace con todo concedido; conceder menos después no revoca nada. | Si quieres negar un permiso, **`revoke`** explícito. Y no fiarse: la RLS es la que de verdad cierra la puerta, el `grant` es el segundo cerrojo. |
| **Borrar en el orden equivocado con `ON DELETE RESTRICT`** | El guion de limpieza dice que ha borrado y no ha borrado nada | Borraba el padre antes que el hijo; la FK lo impedía y **nadie miraba el error** porque el resultado no se comprobaba | En los guiones de mantenimiento, comprobar el error de **cada** borrado, no solo de los inserts. Y ordenar los borrados de hoja a raíz. |
