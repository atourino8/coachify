...
/**
 * Cadenas que NO se tocan aunque contengan el nombre viejo.
 *
 * `coachify_demo` es la marca que llevan en su metadata los usuarios demo YA
 * creados en auth.users. Si se renombra, `seed-demo.mjs --limpiar` deja de
 * reconocerlos y se quedan once usuarios huérfanos que hay que borrar a mano
 * desde Supabase. El identificador es un dato ya escrito en la base, no texto
 * de marca: renombrarlo no arregla nada y rompe algo.
 */
const PROTEGIDOS = ['coachify_demo'];
...
// Los documentos de la raíz se recogen solos: así los ADR y las notas que se
// escriban en el futuro entran sin tener que acordarse de actualizar la lista.
const RAIZ = readdirSync('.').filter(
  (f) => EXTS.has(extname(f)) && !f.startsWith('.') && statSync(f).isFile()
);
...
for (const file of files) {
  ...
  // Las cadenas protegidas se apartan con un centinela antes de sustituir
  // nada, y se devuelven al final. Es más fiable que intentar excluirlas con
  // una expresión regular llena de excepciones.
  const centinelas = PROTEGIDOS.map((p, i) => [p, `__MARCA_PROTEGIDA_${i}__`]);
  for (const [original_, centinela] of centinelas) {
  ...
  for (const [original_, centinela] of centinelas) {
    updated = updated.replaceAll(centinela, original_);
  }
