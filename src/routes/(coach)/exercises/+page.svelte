  ...
  {:else}
    <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
      {#each data.exercises as ex (ex.id)}
        <a href="/exercises/{ex.id}" class="card group hover:border-primary/50 transition-all">
          <div
            class="aspect-video bg-bg rounded-md mb-4 grid place-items-center text-text-mute/40 text-4xl"
          >
            {#if ex.video_url}
              ▶
            {:else}
          ...
          <div class="flex items-center gap-2 text-xs text-text-mute">
            {#if ex.muscle_group}
              <span class="bg-bg px-2 py-1 rounded"
                >{muscleLabels[ex.muscle_group] ?? ex.muscle_group}</span
              >
            {/if}
            {#if !ex.video_url}
