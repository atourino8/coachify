...
<div class="space-y-8">
  ...
  {#if form?.success && form?.added}
    <p aria-live="polite" class="text-sm text-success bg-success/10 border border-success/20 rounded-md p-3">
      {form.added} {form.added === 1 ? 'persona añadida' : 'personas añadidas'} al grupo.
    </p>
  {/if}
  {#if form?.success && form?.programmedGroup}
    <p aria-live="polite" class="text-sm text-success bg-success/10 border border-success/20 rounded-md p-3">
      Programado para {form.clients} {form.clients === 1 ? 'persona' : 'personas'}:
      {form.created} {form.created === 1 ? 'entreno creado' : 'entrenos creados'}{form.skipped > 0
    ...
    <div class="card space-y-4">
      <div>
        ...
        <p class="text-xs text-text-mute mt-0.5">
          Asigna el mismo entrenamiento a las {data.members.length} personas del grupo de una vez.
          Después puedes ajustar a cualquiera individualmente.
        </p>
      </div>
      ...
      >
            ...
            <div class="flex gap-1">
              {#each WEEKDAYS as d (d.v)}
                <button
                  ...
                  class="w-8 h-9 rounded-md text-sm font-medium transition-colors {pgDays.includes(d.v)
        ...
        <button type="submit" disabled={pgSubmitting || !pgTemplate || pgDays.length === 0}
  ...
  <section class="space-y-3">
    <div class="flex items-center justify-between gap-4">
      ...
      <div class="flex items-center gap-3">
        <a href="/clients?invite=1&group={data.group.id}" class="text-sm text-primary hover:underline whitespace-nowrap">
          + Invitar nuevas
        </a>
    ...
    {#if showAdd}
      ...
      >
        <p class="text-xs uppercase tracking-wider text-text-mute">Clientes que aún no están en el grupo</p>
        <div class="grid sm:grid-cols-2 gap-2 max-h-64 overflow-y-auto">
          {#each data.available as c (c.id)}
            <label class="flex items-center gap-2 text-sm bg-bg border border-text-mute/15 rounded-md px-3 py-2 cursor-pointer">
              <input
                type="checkbox"
                name="client_ids"
                value={c.id}
